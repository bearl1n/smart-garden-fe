...
export const deviceApiSlice = apiSlice.injectEndpoints({
    ...
    endpoints: (build) => ({
        ...
        getDeviceByToken: build.query<IDevice[], number>({
            query: (id) => ({ url: `/user/devices` }),
            extraOptions: {
                maxRetries: 8,
                providesTags: ['OrderTag']
            }, // You can override the retry behavior on each endpoint
...
export const { useGetDevicesQuery, useGetDeviceByTokenQuery, useSetOrderMutation } = deviceApiSlice;
